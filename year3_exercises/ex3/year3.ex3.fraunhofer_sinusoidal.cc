...
//		cout << "q = " << q [i] << endl ;
		...
		{
			inp [ 2 * i ] = cos ( 0.5 * M_PHASE * sin ( 2 * M_PI * x [i] / S_PHASE ) ) ;
			inp [ 2 * i + 1 ] = sin ( 0.5 * M_PHASE * sin ( 2 * M_PI * x [i] / S_PHASE ) ) ;
		}
	}
	...
	outfile.open( "fraunhofer_sinusoidal.dat" , ios::trunc ) ;
