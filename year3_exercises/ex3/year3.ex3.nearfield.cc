...
#define N  128 	// size of FT - FFT is fastest when this is a power of 2. Need N > 2 * EXTENT / WIDTH
#define LAMBDA  0.5		// distances measured in microns
#define EXTENT  5000
#define WIDTH  2000
#define SCREEN_DISTANCE  5e3 
#define M_PHASE  8
#define S_PHASE  100
...
//		cout << "q = " << q [i] << endl ;
		...
		// transmission function for sinusoidal grating
		if ( fabs ( x [i] ) <= ( 0.5 * WIDTH ) )
		{
			inp [ 2 * i ] = transmission_phase [ 2 * i ] ;
			inp [ 2 * i + 1 ] = transmission_phase [ 2 * i + 1 ] ;
		}
	}
	...
	outfile.open( "nearfield_slit.dat" , ios::trunc ) ;
